...
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card"
  import { Users, PlusCircle, Clock, Lock, MessageCircle, Unlock, MessageCircleCode, MessageSquareText, XCircle,  } from "lucide-react"
...
export function ConsultationsSummary({ stats }: ConsultationsSummaryProps) {
  const kpiCards = [
    ...
    {
      title: "تم الرد",
      value: stats.answeredConsultations,
      icon: MessageCircle,
      bgColor: "bg-blue-100 dark:bg-blue-900/20",
      iconColor: "text-blue-600 dark:text-blue-400",
      iconBg: "bg-blue-200 dark:bg-blue-900/30",
    }
  ]
      ...
      {kpiCards?.map((item, index) => (
        <Card
          ...
          className={`${item.bgColor} overflow-hidden relative group hover:shadow-lg dark:hover:shadow-gray-900/20 transition-all duration-300 border-0 shadow-md dark:shadow-gray-900/10 ${
            index >= kpiCards.length - 2 ? 'lg:col-span-2' : ''
          }`}
        >
